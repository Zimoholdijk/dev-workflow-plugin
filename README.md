...
| `explain-decision` | Teach ONE technical decision before it is decided: 2-5 prerequisite concepts, one per message (~100 words, cap 130), plain language grounded in the project's actual pages and actors, confirmation gate after each, then a sub-120-word synthesis, and only then the decision itself (recommendation first, unhedged). Invoked directly or from inside `tradeoff-review` when the user says "explain this" |
