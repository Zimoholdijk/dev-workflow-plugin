...
- **Area tag:** label each finding with an area/topic (e.g. "sync state machine", "public API contract") so the test-obligation list, the assessor's banking, and its escalation test can name a consistent area. Use stable, consistent labels across rounds.
...
- pre-judge the escalation question for it (don't assert "the sync bug was a different area than the export bug, so no escalation" — hand it the area-tagged findings and let it compare across rounds),
