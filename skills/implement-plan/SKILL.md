...
3. **Optional self-review.** For a non-trivial phase, run `/code-review` (a Claude Code built-in; if it is unavailable, use `/full-code-review` scoped to this phase) on just this phase's diff before moving on. Tell it to flag only correctness and requirement gaps, not style or speculative hardening (a reviewer told to "find problems" invents them). Catching a bug now is cheaper than at the final full review.
