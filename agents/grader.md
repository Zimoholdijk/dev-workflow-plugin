...
- A **decision** is a choice about the design that could legitimately go more than one way and that a person should own: *which* trust-boundary model, *which* persisted-data shape, *which* contract. It asks "which way should this go?" **"Which methodology or mechanism measures/verifies X" is a decision too**, even though it is reversible: when defensible approaches trade precision against complexity (a simulation vs simple arithmetic, a schedule parser vs a counting query), a person owns that choice. Grading it as a defect-with-one-fix licenses the loop to redesign it autonomously, round after round.
