...
3. **Escalate on structural instability or budget (hard stop).** Three triggers, one verdict:
   ...
   - the same area produces its **third Significant** across the whole run (banked or not; count per area exactly like One-ways): three rounds of prose fixes in one area means the area's *shape* is wrong, and the user chooses the simplification (usually deleting the machinery) — the loop does not re-fix it a fourth time. A dry-signal **reversal** (this round's new-unique count above last round's) in an area already carrying a live Significant is supporting evidence for this trigger, not a trigger by itself; before naming the shape as the root cause, check what recurred: if the area's Significants were claims about live data or an external system that were fixed without a matching entry in the Premises or the plan's Verified Facts, the root cause is that the loop has been fixing unverified guesses, and the decision you hand the user is **resolve by data** (list the facts to check and re-grade from the results), not simplify-or-accept;
